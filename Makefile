...
# The Lua plugin runtime (plugins/nginx-lint-plugin-sdk/runtimes/lua) is C compiled
# with wasi-sdk, committed as runtime.core.wasm and embedded into the
# nginx-lint-plugin-sdk binary. Rebuild it after changing anything under runtimes/lua/
# or the WIT; the Makefile there fetches its pinned toolchain.
build-lua-runtime:
	$(MAKE) -C plugins/nginx-lint-plugin-sdk/runtimes/lua toolchain all
...
# licenses/THIRD-PARTY-NOTICES is what `nginx-lint license` prints: the license
# texts of every crate compiled into the binary, generated by cargo-about from
# about.toml and about.hbs and committed. The features are a superset of what
# any CLI build enables, so the one file covers each of them. `wasm` is not a
# CLI feature, but `web-server-embed-wasm` embeds web/pkg, which is built with
# it; listing it brings that module's crates (wasm-bindgen and the rest) into
# the host's tree, where cargo-about can see them. One gap remains: the
