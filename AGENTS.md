...
(`about.toml`, `about.hbs`) and committed. It names crates without versions,
so a dependency bump alone leaves it unchanged; when a crate enters or leaves
the tree, or a license text changes, CI's `make check-licenses` fails until
`make build-licenses` is run and the result committed. A new dependency under
