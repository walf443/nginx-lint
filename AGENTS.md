...
`nginx-lint license` prints `licenses/THIRD-PARTY-NOTICES`, the license texts
of every crate compiled into the binary, generated by cargo-about
(`about.toml`, `about.hbs`) and committed; `nginx-lint-plugin-sdk license`
prints `plugins/nginx-lint-plugin-sdk/licenses/THIRD-PARTY-NOTICES`, the same
for that binary, after its Lua runtime's hand-kept notices. They name crates
without versions, so a dependency bump alone leaves them unchanged; when a
crate enters or leaves the tree, or a license text changes, CI's
`make check-licenses` fails until `make build-licenses` is run and the result
committed. A new dependency under
a license missing from `accepted` in `about.toml` fails the generation too —
add the license there only once it is known to be compatible with MIT
distribution.
...
the linter checks it — in one call with its siblings, over every member's bad
example — and has to report the same. With `--fixtures` it also runs each
case's `error/` and `expected/`,
